...
func updateBlockMetrics(currentBlock *types.Block, parentHeader *types.Header) error {
	if currentBlock.Number() > 1 {
		...
		metrics.SetGauge(
			...
			float32(headerTime.Sub(parentTime).Seconds()),
		)
	}

	// update the number of transactions in the block metric
	metrics.SetGauge(
		[]string{consensusMetricsPrefix, "num_txs"},
		float32(len(currentBlock.Transactions)),
	)
	...
	metrics.SetGauge(
		[]string{consensusMetricsPrefix, "rounds"},
		float32(extra.Checkpoint.BlockRound),
	)
	...
	metrics.SetGauge(
		[]string{consensusMetricsPrefix, "block_space_used"},
		float32(currentBlock.Header.GasUsed),
	)

	// Update the base fee metric
	metrics.SetGauge(
		[]string{consensusMetricsPrefix, "base_fee"},
		float32(currentBlock.Header.BaseFee),
	)
...
func updateEpochMetrics(epoch epochMetadata) {
	...
	metrics.SetGauge(
		[]string{consensusMetricsPrefix, "validators"},
		float32(epoch.Validators.Len()),
	)
}
