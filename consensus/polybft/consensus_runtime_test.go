...
func TestConsensusRuntime_OnBlockInserted_EndOfEpoch(t *testing.T) {
	...
	blockchainMock.On("GetStateProviderForBlock", mock.Anything).
	...
	polybftBackendMock.On("GetValidatorsWithTx", mock.Anything, mock.Anything, mock.Anything).
		Return(validatorSet).
		Times(3)
...
func Test_NewConsensusRuntime(t *testing.T) {
	...
	blockchainMock.On("CurrentHeader").
		Return(&types.Header{Number: 1, ExtraData: createTestExtraForAccounts(t, 1, validators, nil)})
	blockchainMock.On("GetStateProviderForBlock", mock.Anything).
		Return(new(stateProviderMock), nil).
		Once()
	blockchainMock.On("GetSystemState", mock.Anything, mock.Anything).Return(systemStateMock).Once()
	blockchainMock.On("GetHeaderByNumber", uint64(0)).
		Return(&types.Header{Number: 0, ExtraData: createTestExtraForAccounts(t, 0, validators, nil)})
	blockchainMock.On("GetHeaderByNumber", uint64(1)).
		Return(&types.Header{Number: 1, ExtraData: createTestExtraForAccounts(t, 1, validators, nil)})
	...
	assert.Equal(
		...
		contracts.HydraChainContract.String(),
	)
...
func TestConsensusRuntime_calculateStateTxsInput_SecondEpoch(t *testing.T) {
	...
	polybftBackendMock.On("GetValidators", mock.Anything, mock.Anything).
		Return(validators.GetPublicIdentities()).
		Twice()
...
func TestConsensusRuntime_generateSyncValidatorsDataTxInputRemoved(t *testing.T) {
	t.Parallel()
	...
	extraData := Extra{
		...
		Parent:     &Signature{Bitmap: nil, AggregatedSignature: dummySignature[:]},
		Committed:  &Signature{Bitmap: nil, AggregatedSignature: dummySignature[:]},
	...
	lastBuiltBlock := &types.Header{
		...
		ExtraData:  extraData.MarshalRLPTo(nil),
	...
	valSet := validator.NewValidatorSet(consensusRuntime.epoch.Validators.Copy(), consensusRuntime.logger)
	valSetAccounts := valSet.Accounts()

	parentIbftExtraData, err := GetIbftExtra(lastBuiltBlock.ExtraData)
	assert.NoError(t, err)

	validatorsDelta := parentIbftExtraData.Validators

	expectedUpdatedValidatorsPower := extractUpdatedValidatorsVotingPower(validatorsDelta, valSetAccounts, accSet)
	...
	assert.Equal(t, expectedUpdatedValidatorsPower[0].Validator, syncValidatorsDataInput.ValidatorsPower[0].Validator)
	assert.Equal(t, big.NewInt(0), syncValidatorsDataInput.ValidatorsPower[0].VotingPower)

	polybftBackendMock.AssertExpectations(t)
}

func TestConsensusRuntime_generateSyncValidatorsDataTxInputAdded(t *testing.T) {
	t.Parallel()
	...
	expectedUpdatedValidatorsPower := extractUpdatedValidatorsVotingPower(validatorsDelta, valSetAccounts, nil)
...
func TestConsensusRuntime_IsValidValidator_BasicCases(t *testing.T) {
	...
	setupFn := func(t *testing.T) (*consensusRuntime, *validator.TestValidators) {
		...
		runtime := &consensusRuntime{
			...
			logger: hclog.NewNullLogger(),
			fsm: &fsm{
				validators: validator.NewValidatorSet(epoch.Validators, hclog.NewNullLogger()),
			},
		}
...
func TestConsensusRuntime_TamperMessageContent(t *testing.T) {
	...
	proposalSignature, err := sender.Key().
		SignWithDomain(proposalHash, signer.DomainCheckpointManager)
	require.NoError(t, err)
...
func TestConsensusRuntime_IsValidProposalHash(t *testing.T) {
	...
	require.True(
		t,
		runtime.IsValidProposalHash(
			&proto.Proposal{RawProposal: block.MarshalRLP()},
			proposalHash.Bytes(),
		),
	)
}
...
func TestConsensusRuntime_BuildPrePrepareMessage_EmptyProposal(t *testing.T) {
	...
	assert.Nil(
		t,
		runtime.BuildPrePrepareMessage(
			nil,
			&proto.RoundChangeCertificate{},
			&proto.View{Height: 1, Round: 0},
		),
	)
}
...
func createTestBlocks(
	...
	for i := uint64(1); i <= numberOfBlocks; i++ {
		...
		header := &types.Header{
			Number:     i,
			ParentHash: parentHash,
			ExtraData: createTestExtraForAccounts(
				t,
				getEpochNumber(t, i, defaultEpochSize),
				validatorSet,
				bitmaps[i],
			),
			GasLimit: types.StateTransactionGasLimit,
...
func createTestBitmaps(
	t *testing.T,
	validators validator.AccountSet,
	numberOfBlocks uint64,
) map[uint64]bitmap.Bitmap {
	t.Helper()
...
func createTestExtraForAccounts(
	t *testing.T,
	epoch uint64,
	validators validator.AccountSet,
	b bitmap.Bitmap,
...
func extractUpdatedValidatorsVotingPower(
	validatorsDelta *validator.ValidatorSetDelta,
	currAccSet validator.AccountSet,
	lastEpochValidators validator.AccountSet,
	...
	if validatorsDelta.Added.Len() > 0 || validatorsDelta.Updated.Len() > 0 {
		for _, validator := range currAccSet {
