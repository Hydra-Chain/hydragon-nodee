...
func TestFSM_ValidateHeader(t *testing.T) {
	...
	require.ErrorContains(
		t,
		validateHeaderFields(parent, header, blockTimeDrift),
		"difficulty should be greater than zero",
	)
...
func TestFSM_verifyCommitEpochTx(t *testing.T) {
	...
	// include commit epoch transaction to the epoch starting block
	commitEpochTx, err := fsm.createCommitEpochTx()
	assert.NoError(t, err)
	assert.NotNil(t, commitEpochTx)

	assert.NoError(t, fsm.verifyCommitEpochTx(commitEpochTx, 0))

	// submit tampered commit epoch transaction to the epoch starting block
	...
	assert.ErrorContains(
		t,
		fsm.verifyCommitEpochTx(alteredCommitEpochTx, 0),
		"invalid commit epoch transaction",
	)

	// submit commit epoch transaction that is not the first in the block
	assert.ErrorContains(
		t,
		fsm.verifyCommitEpochTx(commitEpochTx, 1),
		"invalid transaction index. Expected 0, but got 1",
	)

	// submit validators commit epoch transaction to the non-epoch starting block
	fsm.isStartOfEpoch = false
	commitEpochTx, err = fsm.createCommitEpochTx()
	assert.NoError(t, err)
	assert.NotNil(t, commitEpochTx)
	assert.ErrorContains(
		t,
		fsm.verifyCommitEpochTx(commitEpochTx, 0),
		errCommitEpochTxNotExpected.Error(),
	)
}
...
func TestFSM_BuildProposal_EpochStartingBlock_FailedToApplyStateTx(t *testing.T) {
	t.Parallel()
	...
	newValidatorSet := validator.NewValidatorSet(validatorSet, hclog.NewNullLogger())
...
func TestFSM_BuildProposal_EpochEndingBlock_ValidatorsDeltaExists(t *testing.T) {
	...
	for _, removedValidator := range removedValidators {
		assert.True(
			...
			fmt.Sprintf(
				"Expected validator at index %d to be marked as removed, but it wasn't",
				removedValidator,
			),
		)
	}
...
func TestFSM_BuildProposal_NonEpochEndingBlock_ValidatorsDeltaNil(t *testing.T) {
	...
	extra := createTestExtra(
		testValidators.GetPublicIdentities(),
		validator.AccountSet{},
		accountCount-1,
		signaturesCount,
		signaturesCount,
...
func TestFSM_VerifyStateTransactions_StartOfEpochWithoutTransaction(t *testing.T) {
	t.Parallel()
	...
	assert.EqualError(t, fsm.VerifyStateTransactions([]*types.Transaction{}),
		"commit epoch transaction is not found in the epoch starting block")
}

func TestFSM_VerifyStateTransactions_StartOfEpochWrongCommitEpochTx(t *testing.T) {
	t.Parallel()
	...
	commitEpochInput, err := createTestCommitEpochInput(t, 1, allAccounts, 5).EncodeAbi()
	require.NoError(t, err)

	commitEpochTx := createStateTransactionWithData(
		1,
...
func TestFSM_VerifyStateTransactions_StartOfEpochMoreThanOneCommitEpochTx(t *testing.T) {
	t.Parallel()

	testValidators := validator.NewTestValidators(t, 5)
	allAccounts := testValidators.GetPublicIdentities()
	...
	commitEpochTxTwo := createTestCommitEpochInput(t, 0, allAccounts, 100)
...
func TestFSM_VerifyStateTransactions_StartOfEpochFundRewardWalletTx(t *testing.T) {
	t.Parallel()
...
func TestFSM_VerifyStateTransactions_StartOfEpochMissingFundRewardWalletTx(t *testing.T) {
	t.Parallel()
	...
	assert.EqualError(
		t,
		fsm.VerifyStateTransactions([]*types.Transaction{commitEpochTx}),
		"fund reward wallet transaction is not found in the epoch starting block",
	)
}

func TestFSM_VerifyStateTransactions_StartOfEpochWithoutFundRewardWalletTx(t *testing.T) {
	t.Parallel()
...
func TestFSM_VerifyStateTransactions_StartOfEpochWrongDistributeDAOIncentiveTx(t *testing.T) {
	t.Parallel()
...
func TestFSM_VerifyStateTransactions_StartOfEpochMissingDistributeVaultFundsTx(t *testing.T) {
	t.Parallel()
	...
	assert.EqualError(
		...
		"distribute DAO incentive transaction is not found in the epoch starting block",
	)
}

func TestFSM_VerifyStateTransactions_StartOfEpochMissingRewardWalletFundTxWhenDistributeRewards(t *testing.T) {
	t.Parallel()
...
func TestFSM_VerifyStateTransactions_StartOfEpochMissingRewardWalletFundTxWhenDistributeDAOIncentive(t *testing.T) {
	t.Parallel()
...
func TestFSM_VerifyStateTransactions_NonStartOfEpochErrOnCommitEpochTx(t *testing.T) {
	t.Parallel()
	...
	assert.ErrorContains(
		...
		"didn't expect commit epoch transaction in a non epoch starting block",
	)
}
...
func TestFSM_ValidateCommit_InvalidHash(t *testing.T) {
	...
	err = fsm.ValidateCommit(
		validators.GetValidator("0").Address().Bytes(),
		wrongSignature,
		[]byte{},
	)
...
func TestFSM_ValidateCommit_Good(t *testing.T) {
	...
	parent := &types.Header{
		Number:    parentBlockNumber,
		ExtraData: createTestExtra(validatorsMetadata, validator.AccountSet{}, 5, 3, 3),
	}
	parent.ComputeHash()
	stateBlock := createDummyStateBlock(parentBlockNumber+1, parent.Hash, parent.ExtraData)
	mBlockBuilder := newBlockBuilderMock(stateBlock)
...
func TestFSM_Validate_ExitEventRootNotExpected(t *testing.T) {
	...
	validators := validator.NewTestValidators(t, accountsCount)
	parentExtra := createTestExtraObject(
		validators.GetPublicIdentities(),
		validator.AccountSet{},
		4,
		signaturesCount,
	...
	polybftBackendMock.On("GetValidators", mock.Anything, mock.Anything).
		Return(validators.GetPublicIdentities(), nil).
		Once()
	...
	extra.Parent = createSignature(
		t,
		validators.GetPrivateIdentities(),
	...
	stateBlock := createDummyStateBlock(
		parent.Number+1,
		types.Hash{100, 15},
...
func TestFSM_Validate_EpochEndingBlock_MismatchInDeltas(t *testing.T) {
	...
	proposalHash, err := new(
		CheckpointData,
	).Hash(0, stateBlock.Block.Number(), stateBlock.Block.Hash())
	require.NoError(t, err)
...
func TestFSM_Validate_IncorrectHeaderParentHash(t *testing.T) {
	...
	hash, err := new(
		CheckpointData,
	).Hash(fsm.backend.GetChainID(), stateBlock.Block.Number(), stateBlock.Block.Hash())
	require.NoError(t, err)
...
func TestFSM_Validate_TimestampOlder(t *testing.T) {
	...
	for _, blockTime := range []uint64{parent.Timestamp - 2, parent.Timestamp - 1} {
		header := &types.Header{
		...
		stateBlock := &types.FullBlock{
			Block: consensus.BuildBlock(consensus.BuildBlockParams{Header: header}),
		}
		fsm := &fsm{
			parent:     parent,
			backend:    &blockchainMock{},
		...
		checkpointHash, err := new(
			CheckpointData,
		).Hash(fsm.backend.GetChainID(), header.Number, header.Hash)
		require.NoError(t, err)
	...
	setupFn := func() (*fsm, []*messages.CommittedSeal, *types.FullBlock, *blockchainMock) {
		...
		f := &fsm{
			...
			validators: validator.NewValidatorSet(
				validatorsMetadata[0:len(validatorsMetadata)-1],
				hclog.NewNullLogger(),
			),
			logger: hclog.NewNullLogger(),
		}
		...
		for i := 0; i < signaturesCount; i++ {
			sign, err := allAccounts[i].Bls.Sign(
				builtBlock.Block.Hash().Bytes(),
				signer.DomainCheckpointManager,
			)
			require.NoError(t, err)
...
func TestFSM_Insert_InvalidNode(t *testing.T) {
	...
	commitedSeals := []*messages.CommittedSeal{
		...
		{
			Signer:    nonValidatorAccount.Address().Bytes(),
			Signature: nonValidatorSignature,
...
func TestFSM_DecodeCommitEpochStateTx(t *testing.T) {
	...
	commitEpoch := createTestCommitEpochInput(t, 0, allAccounts, 10)
	input, err := commitEpoch.EncodeAbi()
	require.NoError(t, err)
	require.NotNil(t, input)

	tx := createStateTransactionWithData(1, contracts.HydraChainContract, input, nil)
	decodedInputData, err := decodeStateTransaction(tx.Input)
	require.NoError(t, err)

	decodedCommitEpoch, ok := decodedInputData.(*contractsapi.CommitEpochHydraChainFn)
	require.True(t, ok)
...
func TestFSM_Validate_FailToVerifySignatures(t *testing.T) {
	...
	checkpointHash, err := new(
		CheckpointData,
	).Hash(fsm.backend.GetChainID(), finalBlock.Number(), finalBlock.Hash())
	require.NoError(t, err)
...
func createTestExtra(
	...
	extraData := createTestExtraObject(
		allAccounts,
		previousValidatorSet,
		validatorsCount,
		committedSignaturesCount,
		parentSignaturesCount,
	)
...
func createTestExtraObject(allAccounts,
	...
	extraData := &Extra{}
	extraData.Validators = generateValidatorDelta(
		validatorsCount,
		allAccounts,
		previousValidatorSet,
	)
...
func generateValidatorDelta(
	validatorCount int,
	allAccounts, previousValidatorSet validator.AccountSet,
