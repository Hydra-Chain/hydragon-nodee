...
These are used for the communication with the CoinGecko and CoinMarketCap APIs at this moment. This is because we fetch price data from these sources. The configuration file can be generated by running the following command:

```
hydra secrets generate --type local --name node --extra "coingecko-api-key=<key>,coinmarketcap-api-key=<key>"
```

Where the exact keys can be taken from the corresponding website.
